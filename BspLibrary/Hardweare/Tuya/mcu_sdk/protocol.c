...
const DOWNLOAD_CMD_S download_cmd[] =
{
  ...
  {DPID_CH4O_VALUE, DP_TYPE_VALUE},
  {DPID_PM1_VALUE, DP_TYPE_VALUE},
  {DPID_PM2_5_VALUE, DP_TYPE_VALUE},
  {DPID_PM10_VALUE, DP_TYPE_VALUE},
  {DPID_NOISE_VALUE, DP_TYPE_ENUM},
};
...
//    #error "请在此处理可下发可上报数据及只上报数据示例,处理完成后删除该行"
    ...
    mcu_dp_value_update(DPID_TEMP_CURRENT,SensorDataStr.HDCStr.Temperature); //VALUE型数据上报; 温度
    mcu_dp_value_update(DPID_HUMIDITY_VALUE,SensorDataStr.HDCStr.Humidity); //VALUE型数据上报;湿度
...
//    mcu_dp_bool_update(DPID_CHARGE_STATE,当前充电状态); //BOOL型数据上报;
    mcu_dp_enum_update(DPID_TEMP_UNIT_CONVERT,LED); //枚举型数据上报; 温标切换
//    mcu_dp_value_update(DPID_BRIGHT_VALUE,当前当前显示亮度值); //VALUE型数据上报;
    mcu_dp_value_update(DPID_CO2_VALUE,SensorDataStr.CO2Value); //VALUE型数据上报; 二氧化碳浓度
    mcu_dp_value_update(DPID_LIGHT_VALUE,SensorDataStr.Light); //VALUE型数据上报; 光照强度
    mcu_dp_value_update(DPID_CH4O_VALUE,SensorDataStr.WZSStr.WZSCHValue); //VALUE型数据上报; 甲醛浓度
    mcu_dp_value_update(DPID_PM1_VALUE,SensorDataStr.PMSStr.PM1Value); //VALUE型数据上报; PM1浓度
    mcu_dp_value_update(DPID_PM2_5_VALUE,SensorDataStr.PMSStr.PM2_5Value); //VALUE型数据上报; PM2.5浓度
    mcu_dp_value_update(DPID_PM10_VALUE,SensorDataStr.PMSStr.PM10Value); //VALUE型数据上报;  PM10浓度
//    mcu_dp_enum_update(DPID_NOISE_VALUE,SensorDataStr当前); //枚举型数据上报;  噪声等级
...
//    #error "请自行实现wifi功能测试成功/失败代码,完成后请删除该行"
    if(result == 0) {
        //测试失败
        if(rssi == 0x00) {
            LED = 0;//未扫描到名称为tuya_mdev_test路由器,请检查
        }else if(rssi == 0x01) {
           LED=0; //模块未授权
        }
    }else {
...
//  #error "请自行完成获取 WIFI 状态结果代码,并删除该行"
  TuyaWifiInforStr.WifiRunMode = result;//wifi工作状态
