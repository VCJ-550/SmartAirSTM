...
u16 EXW25QXX_TYPE=EXW25Q128;	//默认是W25Q128
...
{ 
 ...
 GPIO_InitTypeDef  GPIO_InitStructure;
	...
	EXW25QXX_CS=1;			//SPI FLASH不选中
	SPI3_Init();		   			//初始化SPI
	SPI3_SetSpeed(SPI_BaudRatePrescaler_2);		//设置为42M时钟,高速模式 
	EXW25QXX_TYPE=EXW25QXX_ReadID();	//读取FLASH ID.
}  
...
{  
	u8 exbyte=0;   
	EXW25QXX_CS=0;                            //使能器件   
	SPI3_ReadWriteByte(EXW25X_ReadStatusReg);    //发送读取状态寄存器命令    
	exbyte=SPI3_ReadWriteByte(0Xff);             //读取一个字节  
	EXW25QXX_CS=1;                            //取消片选     
	return exbyte;   
} 
//写W25QXX状态寄存器
...
{   
	EXW25QXX_CS=0;                            //使能器件   
	SPI3_ReadWriteByte(EXW25X_WriteStatusReg);   //发送写取状态寄存器命令    
...
{
	EXW25QXX_CS=0;                            //使能器件   
    SPI3_ReadWriteByte(EXW25X_WriteEnable);      //发送写使能  
	EXW25QXX_CS=1;                            //取消片选     	      
} 
//W25QXX写禁止	
//将WEL清零  
...
{  
	EXW25QXX_CS=0;                            //使能器件   
    SPI3_ReadWriteByte(EXW25X_WriteDisable);     //发送写禁止指令    
	EXW25QXX_CS=1;                            //取消片选     	      
} 		
//读取芯片ID
//返回值如下:				   
...
{ 
 	u16 i;   										    
	EXW25QXX_CS=0;                            //使能器件   
    SPI3_ReadWriteByte(EXW25X_ReadData);         //发送读取命令   
...
void EXW25QXX_Write_Page(u8* pBuffer,u32 WriteAddr,u16 NumByteToWrite)
{
	...
	EXW25QXX_CS=0;                            //使能器件   
    SPI3_ReadWriteByte(EXW25X_PageProgram);      //发送写页命令   
...
// 	printf("起始地址:0x%0x,字节数:0x%0x\r\n",WriteAddr,NumByteToWrite);//测试用
//	printf("扇区号:0x%0x,扇区内偏移:0x%0x,扇区剩余大小:0x%0x\r\n",secpos,secoff,secremain);//测试用
 	if(NumByteToWrite<=secremain)secremain=NumByteToWrite;//不大于4096个字节
	while(1) 
	{	
		EXW25QXX_Read(EXW25QXX_BUF,secpos*4096,4096);//读出整个扇区的内容
		for(i=0;i<secremain;i++)//校验数据
		{
			if(EXW25QXX_BUF[secoff+i]!=0XFF)break;//需要擦除  	  
		}
		if(i<secremain)//需要擦除
		{
			EXW25QXX_Erase_Sector(secpos);//擦除这个扇区
			for(i=0;i<secremain;i++)	   //复制
			{
				EXW25QXX_BUF[i+secoff]=pBuffer[i];	  
			}
			EXW25QXX_Write_NoCheck(EXW25QXX_BUF,secpos*4096,4096);//写入整个扇区  

		}else EXW25QXX_Write_NoCheck(pBuffer,WriteAddr,secremain);//写已经擦除了的,直接写入扇区剩余区间. 				   
...
{                                   
    ...
    EXW25QXX_Wait_Busy();   
  	EXW25QXX_CS=0;                            //使能器件   
    SPI3_ReadWriteByte(EXW25X_ChipErase);        //发送片擦除命令  
	EXW25QXX_CS=1;                            //取消片选     	      
	EXW25QXX_Wait_Busy();   				   //等待芯片擦除结束
}   
//擦除一个扇区
